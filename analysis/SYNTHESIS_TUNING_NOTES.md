...
Retrieval parity has had a real gate since the packs existed: `golden.json` is
generated by running the Python reference over the packs, and
`GoldenParityTests` replays it through Swift with rank-overlap and score-delta
tolerances. Tonight's work touched no retrieval file, so that gate is intact.

Synthesis had no equivalent, and one copy had already drifted unnoticed:
`runpod/handler.py` carried an inlined paraphrase of the RAG prompt missing
"Do NOT use any prior knowledge" and "Never contradict or override what the
passages say" -- the two sentences that make an answer citable rather than
plausible -- and joined passage headers with `" | "` where every other engine
used `" · "`. Fixed 2026-09-15.

Where the four engines now stand:

| | Worker (local + RunPod) | Swift on-device | Swift PCC |
|---|---|---|---|
| Max passages | 12 (`SYNTH_MAX_K`) | 10 | 12 |
| Per-work cap | none | 2 | 4 |
| Chars per passage | unlimited | 500 | 2,000 |
| Context window | model's own | 4,096 | 32,768 |
| Temperature | 0.3 | 0.2 | 0.2 |
| Prompt | `GUIDE_SYSTEM` | `GUIDE_SYSTEM` | `GUIDE_SYSTEM` |

The packing differences are forced by the context windows and are not drift.
The prompt is now one text: `gutenberg_kg.synthesis_prompts.GUIDE_SYSTEM`,
mirrored in Swift as `SynthesisPrompt.guideInstructions`, with `RAG_SYSTEM`
kept available on both sides (`SYNTH_PROMPT=worker`, or "Worker parity" in
Settings) so the engines can still be compared under the original
instructions.

`tests/test_synthesis_parity.py` enforces it: it reads the Swift source as
text, compares each literal to its Python constant, checks `Budget.worker
.maxPassages` against `SYNTH_MAX_K`, checks both handlers agree on that
number, and fails if either handler inlines a prompt again. No toolchain, no
corpus, no device -- it runs in ordinary CI on every change to either side.
